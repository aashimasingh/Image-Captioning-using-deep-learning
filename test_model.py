...
def test_model(weight, img_name, beam_size = 1):
...
def test_model_on_images(weight, img_dir, beam_size = 1):
	imgs = []
	captions = {}
	with open(img_dir, 'rt') as f_images:
		imgs = f_images.read().strip().split('\n')
	encoded_images = pickle.load( open( "encoded_images_resnet.p", "rb" ) )
	model = cg.create_model(ret_model = True)
	model.load_weights(weight)

	f_pred_caption = open('predicted_captions.txt', 'wt')
	...
	f_captions = open('Flickr8k_text/Flickr8k.token.txt', 'rt')
...
if __name__ == '__main__':
	weight = 'weights-improvement-resnet-10.hdf5'
	...
	print (test_model_on_images(weight, test_img_dir, beam_size=1))

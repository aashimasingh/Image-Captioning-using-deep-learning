...
from resnetkeras2 import resnet152_model
import numpy as np 
import io
from keras.applications.imagenet_utils import preprocess_input	

counter = 0
weights_path = 'C:/Users/aashi/Downloads/resnet152_weights_tf.h5'
...
def prepare_dataset(no_imgs = -1):
	f_train_images = io.open('Flickr8k_text/Flickr_8k.trainImages.txt','rt')
	train_imgs = f_train_images.read().strip().split('\n') if no_imgs == -1 else f_train_images.read().strip().split('\n')[:no_imgs]
	f_train_images.close()

	f_test_images = io.open('Flickr8k_text/Flickr_8k.testImages.txt','rt')
	test_imgs = f_test_images.read().strip().split('\n') if no_imgs == -1 else f_test_images.read().strip().split('\n')[:no_imgs]
	f_test_images.close()

	f_train_dataset = io.open('Flickr8k_text/flickr_8k_train_dataset.txt','wt')
	f_train_dataset.write("image_id\tcaptions\n")

	f_test_dataset = io.open('Flickr8k_text/flickr_8k_test_dataset.txt','wt')
	f_test_dataset.write("image_id\tcaptions\n")

	f_captions = io.open('Flickr8k_text/Flickr8k.token.txt', 'rt')
